...
<script type="text/javascript">
	function checkGoods(uid){
		$.post("rent/adminindex!checkfGoods",{uid:uid},
				function(checkFlag){
					var flag=eval(checkFlag);
					if(flag){
						alert("物品设置为已交换！");
						window.location.href="rent/adminindex!payGoods?uid="+uid;
					}else{
						alert("物品设置为已交换失败！");
					}
				}
	...
	function checkInfo(uid){
		$.post("rent/adminindex!checktfGoods",{uid:uid},
				function(checkFlag){
					var flag=eval(checkFlag);
					if(flag){
						alert("物品可以重新交换！");
...
<body>
    <center>
        ...
        <table border="0" width="645" cellpadding="10" cellspacing="0" style="word-break:break-all">
            ...
            <tr height="30">
                <td style="text-indent:5"><b>交换物品 [ID值：${goods.uid}]</b><img src="<c:url value='/goods_img/${goods.imagea }'/>" border="0" /></a></td>
            ...
            <tr height="40" bgcolor="#F9F9F9">
                ...
                <td align="center">
                	<input type="button" onclick="checkGoods('${goods.uid}')" value="√已被交换" ${goods.payfor==0?"disabled":"" }/>
                	<input type="button" onclick="checkInfo('${goods.uid}')" value="√可重交换" ${goods.payfor==1?"disabled":"" }/>
                </td>
                <td align="center"></td>                
            </tr>
